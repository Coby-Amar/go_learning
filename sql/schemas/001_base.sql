...
CREATE TABLE _reports (
    ...
    _carbohydrates_total NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_carbohydrates_total > -1), 
    _proteins_total NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_proteins_total > -1),
    _fats_total NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_fats_total > -1),
    _user_id UUID NOT NULL,
    FOREIGN KEY (_user_id) REFERENCES _users(_id) ON DELETE CASCADE ON UPDATE NO ACTION
 );
...
CREATE TABLE _report_entries (
    ...
    _updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
    _amount SMALLINT NOT NULL CHECK(_amount > 0),
    _carbohydrates NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_carbohydrates > -1), 
    _proteins NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_proteins > -1),
    _fats NUMERIC(4,2) NOT NULL DEFAULT 0.0 CHECK(_fats > -1),
